...
docker run -d --name=lcogt_mezzanine_nginx -p 8100:8100 -e PREFIX=$PREFIX $DEBUGENV --link lcogt_mezzanine_uwsgi:lcogt_mezzanine_uwsgi lcogtwebmaster/lcogt:lcogt_mezzanine_$BRANCH /var/www/apps/lcogt_mezzanine/docker/bin/nginx.sh
if [ "$DEBUG" != "" ]; then
